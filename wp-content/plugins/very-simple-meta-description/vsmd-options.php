...
function vsmd_admin_init() {
	...
	add_settings_field( 'vsmd-field-9', esc_attr__( 'Uninstall', 'very-simple-meta-description' ), 'vsmd_field_callback_9', 'vsmd', 'vsmd-section-1' );
	register_setting( 'vsmd-options', 'vsmd-setting-9', array('sanitize_callback' => 'sanitize_key') );
	add_settings_field( 'vsmd-field', esc_attr__( 'Meta Description', 'very-simple-meta-description' ), 'vsmd_field_callback', 'vsmd', 'vsmd-section-1' );
	register_setting( 'vsmd-options', 'vsmd-setting', array('sanitize_callback' => 'sanitize_text_field') );
	...
	if ( class_exists( 'woocommerce' ) ) {
		...
		add_settings_field( 'vsmd-field-8', esc_attr__( 'Description', 'very-simple-meta-description' ), 'vsmd_field_callback_8', 'vsmd', 'vsmd-section-5' );
		register_setting( 'vsmd-options', 'vsmd-setting-8', array('sanitize_callback' => 'sanitize_key') );
	}
}
...
function vsmd_section_callback_1() {
	...
	echo '<li>'.esc_attr__( 'Using the same meta description for every post and page is not SEO friendly.', 'very-simple-meta-description' ).'</li>';
	echo '</ul>';
}

function vsmd_section_callback_2() {
	echo '<ul>';
	echo '<li>'.esc_attr__( 'While adding a post or page you can set an excerpt by using the "Excerpt" box.', 'very-simple-meta-description' ).'</li>';
	echo '<li>'.esc_attr__( 'The "Excerpt" can be used as meta description for that post or page.', 'very-simple-meta-description' ).'</li>';
	echo '<li>'.esc_attr__( 'If you have set a static page as homepage, the excerpt of that page will override the meta description from above.', 'very-simple-meta-description' ).'</li>';
	echo '</ul>';
}

function vsmd_section_callback_3() {
	echo '<ul>';
	echo '<li>'.esc_attr__( 'While adding a category or tag you can set a description by using the "Description" box.', 'very-simple-meta-description' ).'</li>';
	echo '<li>'.esc_attr__( 'The "Description" can be used as meta description for that category or tag page.', 'very-simple-meta-description' ).'</li>';
	echo '</ul>';
}

function vsmd_section_callback_4() {
	echo '<ul>';
	echo '<li>'.esc_attr__( 'While adding a product you can set an excerpt by using the "Product short description" box.', 'very-simple-meta-description' ).'</li>';
...
// field callbacks
function vsmd_field_callback_9() {
	$value = get_option( 'vsmd-setting-9' );
	?>
	<input type='hidden' name='vsmd-setting-9' value='no'>
	<label><input type='checkbox' name='vsmd-setting-9' <?php checked( esc_attr($value), 'yes' ); ?> value='yes'> <?php esc_attr_e( 'Do not delete settings.', 'very-simple-meta-description' ); ?></label>
	<?php
}

function vsmd_field_callback() {
	$value = get_option( 'vsmd-setting' );
	$count = mb_strlen( $value, 'UTF-8' );
	echo '<textarea name="vsmd-setting" rows="8" cols="50" maxlength="500">'.esc_attr($value).'</textarea>';
...
function vsmd_field_callback_8() {
	$value = get_option( 'vsmd-setting-8' );
	?>
	<input type='hidden' name='vsmd-setting-8' value='no'>
	<label><input type='checkbox' name='vsmd-setting-8' <?php checked( esc_attr($value), 'yes' ); ?> value='yes'> <?php esc_attr_e( 'Use as meta description.', 'very-simple-meta-description' ); ?></label>
	<?php
}
